...
from lib.models.utils.image_utils import resize_image
from lib.models.utils.mask_utils import resize_mask, minimize_mask
from lib.models.utils.box_utils import extract_bboxes, compute_overlaps, box_refinement
from lib.models.utils.image_utils import compose_image_meta, subtract_mean, resize
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    :param image_shape:     [H, W, C] with resized image
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    ...
    :return:                [count, (y1, x1, y2, x2)] ROI boxes in pixels.
    '''
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    overlaps = compute_overlaps(rpn_rois, gt_boxes)
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    :param dataset:         The Dataset object to pick data from
    :param config:          The model config object
    :param shuffle:         If True, shuffles the samples before every epoch
    :param augment:         (deprecated. Use augmentation instead). If true, apply random
                            image augmentation. Currently, only horizontal flipping is offered.
    ...
    while True:
        try:
            ...
            batch_images[b] = subtract_mean(image.astype(np.float32), config)
