...
def parse_image_meta_graph(meta):
    ...
    :return: a dict of the parsed tensors.
    '''
...
def resize_image(image, min_dim=None, max_dim=None, min_scale=None, mode="square"):
    ...
    :param image:   [H, W, C]
    :param min_dim: if provided, resizes the image such that it's smaller
        dimension == min_dim
...
def mold_image(image, config):
    '''
    resize and subtract mean of the input image
    :param image:   [H, W, C]
    :param config:  instance of (sub-class of Config)
    :return: molded_image, window, scale, padding, crop
        molded_image: the resized image and then subtract mean
        window: (y1, x1, y2, x2). If max_dim is provided, padding might
            be inserted in the returned image. If so, this window is the
            coordinates of the image part of the full image (excluding
            the padding). The x2, y2 pixels are not included.
        scale: The scale factor used to resize the image
        padding: Padding added to the image [(top, bottom), (left, right), (0, 0)]
        crop: None or (y, x, min_dim, min_dim)
    '''
